...
const navigation: NavItem[] = [
  ...
  { name: "Keuangan", href: "/finance/reports", icon: Wallet },
...
export function SideNavBar({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "flex h-full w-[260px] flex-col bg-primary text-primary-foreground",
        className,
      )}
    >
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        {navigation.map((item) => {
          ...
          const isActive =
            (item.href && pathname === item.href) || isParentActive;
